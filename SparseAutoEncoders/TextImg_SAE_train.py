...
HIDDEN_DIM = 256 # Should not exceed 512
LEARNING_RATE = 1e-3
EPOCHS = 50
BATCH_SIZE = 16
SPARSITY_TARGET = 0.05  # Desired level of sparsity
SPARSITY_WEIGHT = 1e-3   # penalize non-sparsity
K_FOLDS = 3
...
class EmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        embedding = torch.load(self.file_paths[idx])
...
def train_and_evaluate():
    ...
    if not embedding_files:
        print("No embedding files found. Please run 01_text_img_embedding.py first.")
        return
