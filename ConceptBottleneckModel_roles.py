...
# CONFIGURATION
# ========================================================================
DATASET_ANNOTATION_FILE = 'dataset_annotation_output_roles.json'
MERGED_CONCEPTS_FILE = 'merged_concepts_output_roles.json'
EMBEDDING_DIR = 'cropped_embedding_artifacts'
OUTPUT_DIR = 'CBM_artifacts_for_roles'
...
class MaskedBCELoss(nn.Module):
    ...
    def forward(self, concept_logits, concept_labels, mask):
        ...
        """
        # Only compute loss on annotated positions to avoid inf*0=NaN
        mask_bool = mask > 0
        num_annotated = mask_bool.sum()
        if num_annotated == 0:
            return (concept_logits * 0).sum()

        logits_masked = concept_logits[mask_bool]
        labels_masked = concept_labels[mask_bool]
        weights_masked = self.concept_weights.unsqueeze(0).expand_as(concept_logits)[mask_bool]

        bce = nn.functional.binary_cross_entropy_with_logits(
            logits_masked, labels_masked, reduction='none'
        )

        return (bce * weights_masked).sum() / num_annotated
...
def load_and_prepare_data():
    ...
    # Normalize helper: lowercase, strip trailing period, strip whitespace
    def normalize_name(name):
        return name.lower().rstrip('.').strip()

    # Create mappings with normalized names for fuzzy matching
    concept_names = list(merged_concepts.keys())
    num_concepts = len(concept_names)
    concept_to_idx = {c: i for i, c in enumerate(concept_names)}
    norm_concept_to_idx = {normalize_name(c): i for i, c in enumerate(concept_names)}
    
    # Extract class labels from embedding filenames (role-based)
    # Example: VOA_EN_NW_2014.10.09.2478154_0_Entity_1.pt → "Entity"
    class_labels_set = set()
    for embed_name in annotations.keys():
        stem = embed_name.replace('.pt', '')
        parts = stem.split('_')
        if len(parts) >= 2:
            role = parts[-2]
            class_labels_set.add(role)
    ...
    for embed_idx, (embed_name, data) in enumerate(annotations.items()):
        ...
        try:
            ...
            # ==========================================
            # EXTRACT CLASS LABEL (role-based)
            # ==========================================
            # Get role from embedding filename
            # Example: VOA_EN_NW_2014.10.09.2478154_0_Entity_1.pt → "Entity"
            stem = embed_name.replace('.pt', '')
            role = stem.split('_')[-2]
            class_labels_list.append(class_to_idx[role])
            ...
            norm_name = normalize_name(concept_name)
            
            # Check if this concept is in our merged concepts list
            concept_idx = norm_concept_to_idx.get(norm_name)
...
def train_classifier(model_clf, train_logits, train_labels, val_logits, val_labels,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            print(f"  Epoch {epoch+1:2d}/{num_epochs} - Train Loss: {train_loss:.6f}, Val Loss: {val_loss:.6f}, Val Acc: {val_acc:.4f}")
...
def train_with_kfold(embeddings_array, concept_labels_array, class_labels_array,
    ...
    Returns:
        fold_results: list of validation accuracies (one per fold)
    """
